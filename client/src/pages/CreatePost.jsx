...
const CreatePost = () => {
  ...
  if(form.prompt && form.photo){
    ...
    alert('please entrer a prompt and generate an image')
  }
    ...
    if(form.prompt){
      try {
        setGeneratingImg(true)
         // on your local machine you can use : http://localhost:8080/api/v1/post
        const response = await fetch('https://dalle-rh64.onrender.com/api/v1/dalle',{
          method : 'POST',
          headers :{
            'Content-Type': 'application/json',
          },
  ...
  return (
    <section className='max-w-7xl mx-auto'>
      <div>
        ...
        <p className='mt-2 text-[#666e75] text-[16px] max-w-[500px]'>Create imaginative and visually stunning images through DALL-E A and share them with the Community</p>
      </div>
      ...
      <form className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
        ...
        <div className='mt-5 flex gap-5'>
          <button type='button' onClick= {generateImg} className="text-white
          bg-green-700 font-medium rounded-md text-sm w-full sm:w-auto px-5 py-2.5 text-center">
            {
